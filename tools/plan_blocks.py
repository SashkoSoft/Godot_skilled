...
CLOSETS = [   # ниша, дверца вправо (к входной двери)
    ...
    # Кладовка примыкает к стене санузла ровно там, где та стена
    # заканчивается (z=-0.98), и глубиной чуть меньше 1а/2а — не тянется
    # до угла коридора у входной двери. До правки кладовка забирала
    # z вплоть до -0.06, и заодно вычетом отъедала кусок пола прихожей
    # в этой же полосе по X (см. subtract_closets) там, где никакой
    # кладовки на чертеже уже нет.
    ("6а", -11.90, -0.98, -11.44, -0.53, "x+"),
]
