...
const EXIT_W := 1.40
## Насколько бетонная площадка выступает за габарит дома. Было 5 м — это
## заглушка тех времён, когда снаружи не было ничего и дому надо было на чём-то
## стоять. С появлением `street.gd` пятиметровая подушка стала вредной: она
## накрывала собой отмостку с палисадником (улица кладётся ниже, на −0.15,
## а площадка на −0.15..0.05) и превращала дом в макет на постаменте.
## 1.2 м — это уже настоящая отмостка, всё остальное снаружи делает улица.
const APRON := 1.2
