...
func _tex(dir_: String, base: String, scale: float,
	...
	# Без этого триплanar считается в локальных координатах КАЖДОГО отдельного
	# бокса — а стена собрана не одним мешем, а десятками _box() (сегменты
	# по проёмам, перемычка над дверью отдельно от стены слева и справа).
	# У каждого свой локальный ноль, и текстура на стыке визуально «режется»:
	# ровно та же побелка резко меняет масштаб и фазу на границе куска,
	# читается как отдельная заплатка, а не одна стена. world_triplanar
	# считает от мировых координат — стык между любым числом боксов с этим
	# материалом становится бесшовным сам по себе, чинить каждый стык не надо.
